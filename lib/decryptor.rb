...
class Decryptor
    attr_reader :output, :input, :map_index_values, :character_map, :chars_in_message, :values, :offset_values, :rotated_map, :decrypted_message, :offset_calc

    def initialize(input, key, date = nil)
      @decrypted_message = ""
      @input = input
      @map_index_values = []
      @values = []
    ...
    def decryptor
      ...
      p "Created 'decrypted.txt' with the key #{offset_calc.key} and date #{offset_calc.date} "
    end
